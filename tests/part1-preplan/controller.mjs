...
// ---------------------------------------------------------------------------
// Q11 — OLD retirement fence commits on the same held connection
// ---------------------------------------------------------------------------

await runScenario('q11_old_fence_same_held_connection_commit_then_legacy_fail_closed', async () => {
  const oldPath = dbPath('q11-old-fence');
  const old = new DatabaseSync(oldPath);
  old.exec(`
    CREATE TABLE knowledge_meta(key TEXT PRIMARY KEY, value TEXT NOT NULL) STRICT;
    CREATE TABLE business_probe(id INTEGER PRIMARY KEY, value TEXT NOT NULL);
    INSERT INTO knowledge_meta(key,value) VALUES
      ('product_foundation_version','b1'),
      ('product_schema_version','3'),
      ('library_id','q11-library');
  ...
  assert(contenderBusy, 'OLD exclusive did not block contender before fence commit');

  const changed = old.prepare(
    "UPDATE knowledge_meta SET value=? WHERE key='product_foundation_version'"
  ).run('retired-libraryroot-v1');
  assert(Number(changed.changes) === 1, 'retirement fence row was not updated exactly once', changed);

  old.exec('COMMIT');
  old.close();

  const legacy = new DatabaseSync(oldPath);
  const foundation = legacy.prepare(
    "SELECT value FROM knowledge_meta WHERE key='product_foundation_version'"
  ).get()?.value;

  let admission = 'ACCEPTED';
  if (foundation !== 'b1') {
    admission = 'PRODUCT_FOUNDATION_VERSION_UNSUPPORTED';
  } else {
    legacy.prepare("INSERT INTO business_probe(value) VALUES('legacy-write')").run();
  }

  const businessWrites = Number(scalar(
    legacy.prepare('SELECT COUNT(*) FROM business_probe').get()
  ));
  legacy.close();

  assert(
    foundation === 'retired-libraryroot-v1',
    'durable OLD retirement fence missing after COMMIT',
    { foundation }
  );
  assert(
    admission === 'PRODUCT_FOUNDATION_VERSION_UNSUPPORTED',
    'immutable legacy admission did not fail closed',
    { admission }
  );
  assert(
    businessWrites === 0,
    'legacy business write occurred after retirement fence',
    { businessWrites }
  );
